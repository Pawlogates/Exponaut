...
@export var outline_simple : bool = false
...
func _ready() -> void:
	...
	if outline_enabled:
		for x in range(1, outline_part_quantity + 1):
			var outline_segment : Node = get_node("outline" + str(x))
			if outline_width != 1 : outline_segment.position *= outline_width - 1
			var outline_s_segment : Node = get_node("outline_s" + str(x))
			if outline_width != 1 : outline_s_segment.position *= outline_width - 1
		...
		for x in range(1, outline_part_quantity + 1):
			...
			outline_segment.modulate *= randf_range(outline_min_opacity, 100)
			outline_segment.modulate.a = randf_range(outline_min_opacity, 1.0)
