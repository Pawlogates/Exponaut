...
func _process(delta):
	...
	%menu_deco_bg.size.x = move_toward(%menu_deco_bg.size.x, target_W, speed_W * speed * delta * 100)
	%menu_deco_bg.size.y = move_toward(%menu_deco_bg.size.y, target_H, speed_H * speed * delta * 100)
	...
	if toggle:
		skew_W = move_toward(skew_W, skew_target, 3 * delta)
		skew_H = move_toward(skew_H, skew_target, 3 * delta)
	else:
		skew_W = move_toward(skew_W, skew_target, 3 * delta)
		skew_H = move_toward(skew_H, skew_target, 3 * delta)
	...
	if toggle:
		%menu_deco_bg.scale.x = move_toward(%menu_deco_bg.scale.x, scale_low_X, speed_W * 0.06 * delta)
		%menu_deco_bg.scale.y = move_toward(%menu_deco_bg.scale.y, scale_low_Y, speed_H * 0.3 * delta)
	else:
		%menu_deco_bg.scale.x = move_toward(%menu_deco_bg.scale.x, scale_high_X, speed_W * 0.06 * delta)
		%menu_deco_bg.scale.y = move_toward(%menu_deco_bg.scale.y, scale_high_Y, speed_H * 0.3 * delta)
